...
def run_B_once():
    # Richiede che il gestore B supporti --once
    cmd = [PYTHON_B, B_FILE, "--once"]
    logger.info(f"Eseguo B (once): {cmd}")
    try:
        subprocess.run(cmd, cwd=os.path.dirname(B_FILE), check=True)
        logger.info("B OK")
        write_last_b(datetime.now())
        return True
    except subprocess.CalledProcessError as e:
        logger.error(f"B exit {e.returncode}")
    except Exception as e:
